<template>
  ...
  <main class="layout">
    <section class="panel form">
      <component
        :is="active==='cv1' ? CVFormA : active==='cv2' ? CVFormB : active==='cv3' ? CVFormC : NotFound"
        :modelValue="cv"
    ...
    <section class="panel preview">
      <component
        :is="active==='cv1' ? TemplateA : active==='cv2' ? TemplateB : active==='cv3' ? TemplateC : NotFound"
        :data="cv"
      />
      <div class="actions">
...
import CVFormA from '@/components/CVFormA.vue'
...
function mapCvFromBackend(payload) {
  ...
  const awards = (payload.awards || []).map(a => ({
    name: a.name || '',
    org: a.organization || '',
    year: a.year || ''
  }))
  ...
  const certs = (payload.certificates || []).map(c => ({ name: c.name || '', issuer: c.issuer || '', date: formatDate(c.date) || '', url: c.url }))
  ...
  const exps = (payload.experience || []).map(exp => {
    ...
    return {
      ...
      project: projRaw
        ? {
            name: projRaw.name || '',
            period_start: formatDate(projRaw.period_start) || '',
            period_end: formatDate(projRaw.period_end) || '',
  ...
  return {
    ...
    awards: awards.length ? awards : [{ name: '', org: '', year: '' }]
