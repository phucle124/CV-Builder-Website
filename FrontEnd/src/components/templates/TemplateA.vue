	...
	<div class="cv">
		<aside class="sidebar">
			...
			<section class="skills">
				...
				<div class="group">
					...
					<ul class="badges">
						<li v-for="(s,i) in d.skills.backend" :key="'b'+i"><span class="pill" v-show="s.label || s.note">{{ s.label }}</span> <span class="note">{{ s.note }}</span></li>
					</ul>
				</div>
				<div class="group">
					<div class="title">Front-end</div>
					<ul class="badges">
						<li v-for="(s,i) in d.skills.frontend" :key="'f'+i"><span class="pill" v-show="s.label || s.note">{{ s.label }}</span> <span class="note">{{ s.note }}</span></li>
					</ul>
				</div>
				<div class="group">
					<div class="title">Soft Skills</div>
					<ul class="badges">
						<li v-for="(s,i) in d.skills.soft" :key="'s'+i"><span class="pill" v-show="s">{{ s }}</span></li>
					</ul>
				</div>
			</section>
        ...
        <div class="job" v-for="(x, i) in props.data.experience" :key="i">
          ...
          <div v-if="x.project" class="project">
			...
			<div v-if="x.project.description" class="desc">
			...
			<div v-if="x.project.tech" class="tech">Tech stack: {{ x.project.tech }}</div>
			<div v-if="x.project.teamSize" class="team">Team size: {{ x.project.teamSize }} members</div>
			</div>
        </div>
      </section>
        ...
        <div v-for="(h,i) in props.data.highlights" :key="i" class="row">
		  ...
		  &emsp;
          <div class="period">{{ formatDate(h.period_start) }} - {{ formatDate(h.period_end) }}</div>
          <ul class="bullets">
...
.job { margin-top: 14px; }
.row { display: flex; justify-content: space-between; align-items: baseline; padding: 10px;}
