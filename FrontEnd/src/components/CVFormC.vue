...
const props = defineProps({ modelValue: { type: Object, required: true }, cvId: { type: Number, default: null } })
const emit = defineEmits(['update:modelValue'])
const cv = computed({ get: () => props.modelValue, set: (v) => emit('update:modelValue', v) })
...
/**
 * Sử dụng FileReader để chuyển tệp sang Base64 Data URL
 */
function onFile(e) {
...
<template>
  <form class="cv-form" @submit.prevent>
    <h2>Thông tin chung (CV 3)</h2>
    <div class="grid">
      <label>Họ tên<input :value="modelValue.header.name" @input="updateCV('header.name', $event.target.value)" /></label>
      <label>Chức danh<input :value="modelValue.header.title" @input="updateCV('header.title', $event.target.value)" /></label>
    </div>
    <label>Tóm tắt<textarea :value="modelValue.header.summary" @input="updateCV('header.summary', $event.target.value)" rows="4" /></label>
    ...
    <div class="grid">
      ...
      <label>Backend 1 ghi chú<input :value="modelValue.skills.backend[0].note" @input="updateCV('skills.backend.0.note', $event.target.value)" /></label>
      ...
      <label>Frontend 2 <input :value="modelValue.skills.frontend[1].label" @input="updateCV('skills.frontend.0.label', $event.target.value)" /></label>
      <label>Frontend 2 ghi chú<input :value="modelValue.skills.frontend[1].note" @input="updateCV('skills.frontend.0.note', $event.target.value)" /></label>
    ...
    <div class="grid">
      ...
      <label>Link<input :value="modelValue.certificates[0].url" @input="updateCV('certificates.0.url', $event.target.value)" placeholder="https://..." /></label>
    </div>
    ...
    <h2>Ngôn ngữ</h2>
    <div class="grid">
      <label>Ngôn ngữ 1<input :value="modelValue.languages[0]" @input="updateCV('languages.0', $event.target.value)" /></label>
      <label>Ngôn ngữ 2<input :value="modelValue.languages[1]" @input="updateCV('languages.1', $event.target.value)" /></label>
    </div>

    <h2>Giải thưởng</h2>
    <div class="grid">
      <label>Tên giải
  <input :value="modelValue.awards?.[0]?.name || ''"
         @input="updateCV('awards.0.name', $event.target.value)" />
</label>
<label>Tổ chức
  <input :value="modelValue.awards?.[0]?.org || ''"
         @input="updateCV('awards.0.org', $event.target.value)" />
</label>
<label>Năm
  <input :value="modelValue.awards?.[0]?.year || ''"
         @input="updateCV('awards.0.year', $event.target.value)" />
</label>
    </div>
...
label { color: #111820; font-size: 14px; display: grid; gap: 6px; }
input, textarea { background: #ffffff; border: 1px solid #d5dde6; color: #111820; border-radius: 10px; padding: 10px 12px; box-shadow: none; }
